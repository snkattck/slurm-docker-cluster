...
@test 'check slurm version matches SLURM_TAG' {
    run docker exec slurmctld slurmctld -V
...
EOF
    ...
    run docker exec slurmctld sinfo -o '%D' --noheader
    assert_output --regexp "^2$"
}

@test 'job can be submitted' {
    run docker exec slurmctld bash -c "cd /data; sbatch --wrap='echo L' -o out.log"
...
EOF
    ...
    run docker exec slurmctld cat /data/out.log
